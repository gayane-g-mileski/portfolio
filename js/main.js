  ...
  qsa('[data-carousel]').forEach(root => {
    ...
    if (root.hasAttribute('data-carousel-book')) {
      ...
      const setLeaf = (p) => {
        ...
        // a hand does not turn a page about a perfectly upright axis
        const ax = (dir > 0 ? -TILT : TILT) * Math.sin(Math.PI * Math.min(1, p));
        leaf.style.transform = `rotate3d(${ax}, 1, 0, ${deg}deg)`;
        leaf.style.setProperty('--p', String(p));
        under.style.setProperty('--p', String(p));
        // the sheet is darkest edge-on, halfway through the turn
        leaf.style.setProperty('--shade', String(0.16 + 0.62 * (1 - Math.abs(1 - 2 * p))));
      };
